...
//        Date createDate = dateFormat.parse(createDateParam);
                ...
                for (int i = 0; i < items.size(); ++i) {
                    ...
                    if (item.isFormField()) {  // item이 일반 데이터인 경우                      
                        if(item.getFieldName().equals("isPublic")) 
                            ...
                            content = value; 
                        else if(item.getFieldName().equals("locationAddress")) 
                            locationAddress = value; 
                        else if(item.getFieldName().equals("locationName")) 
                            locationName = value; 
                    }
                    else {  // item이 파일인 경우   
     ...
     // 세션에서 사용자 ID를 가져오기
        ...
        int locationId = manager.searchLocationId(locationAddress);
                
        if (locationId == 0) {  //DB에 해당 장소 저장 안되어있음
            locationId = manager.saveLocationId(locationName, locationAddress);
        }
        
        System.out.println("locationId값입니다: " + locationId);
...
//            "image",
        ...
        try {
            ...
            log.debug("getPostId Post : {}", post.getPostId());
